...
async function handleInactivityTimeout() {
    ...
    try {
        const res = await fetch(`${API}/forfeit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameId }) });
        const data = await res.json();
        if (!res.ok || data.error) { showError(String(data.error ?? "Round timed out.")); inactivityResolving = false; return; }
        clearTimeout(inactivityTimer!);
        const priorBet = currentBet ?? 0;
        betInput.disabled = false; startBtn.disabled = false; cashoutBtn.disabled = true;
        setBalance(typeof data.balance === "number" ? data.balance : balance);
        updatePotentialDisplay(0);
        gameOver = true; revealBtn.disabled = true; safeRevealCount = 0; currentBet = null;
        if (Array.isArray(data.mines)) applyMineReveal(data.mines);
        setEarningsDisplay(typeof data.earnings === "number" ? data.earnings : -priorBet);
        if (typeof data.serverSeed === "string") serverSeedEl.textContent = data.serverSeed;
        renderBoard();
    } catch { showError("Forfeit failed."); } finally { inactivityResolving = false; }
}
...
async function startGame() {
    ...
    clearMessage(); startBtn.disabled = true; cashoutBtn.disabled = true;
    try {
        ...
        gameId = data.gameId; clientSeed = data.clientSeed ?? clientSeed; currentBet = data.bet ?? betAmount;
        safeRevealCount = 0; gameOver = false; revealBtn.disabled = true; cashoutBtn.disabled = false; betInput.disabled = true;
        serverSeedEl.textContent = ""; commitEl.textContent = data.commit; clientSeedEl.textContent = clientSeed;
        resetBoard(width * height); updateNonceDisplay(data.nonce);
        setBalance(typeof data.balance === "number" ? data.balance : balance);
        ...
        showInfo("Game armed. Choose wisely."); renderBoard(); armInactivityCountdown();
...
async function clickTile(index: number) {
    ...
    clearTimeout(inactivityTimer!);
    try {
        const res = await fetch(`${API}/revealTile`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameId, tile: index }) });
        const data = await res.json();
        if (data.error) { showError(String(data.error)); return; }
        if (typeof data.nonce === "number") updateNonceDisplay(data.nonce);
        if (data.result === "mine") {
            board[index] = "mine"; if (Array.isArray(data.mines)) applyMineReveal(data.mines);
            gameOver = true; revealBtn.disabled = false; cashoutBtn.disabled = true; betInput.disabled = false; startBtn.disabled = false;
            updatePotentialDisplay(0); safeRevealCount = 0; currentBet = null;
            setEarningsDisplay(typeof data.earnings === "number" ? data.earnings : -(currentBet ?? 0)); showError("Boom! You lost.");
        } else {
            board[index] = "safe"; safeRevealCount = typeof data.safeReveals === "number" ? data.safeReveals : safeRevealCount + 1;
            ...
            armInactivityCountdown(); showSuccess(`Safe tile! ${safeRevealCount} revealed.`);
        }
        renderBoard();
    } catch { showError("Could not reveal tile."); if (!gameOver && gameId) armInactivityCountdown(); }
}

async function cashOut() { /* same as before */ }
async function revealSeed() { /* same as before */ }
