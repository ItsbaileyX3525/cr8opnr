...
const EXP_DIVISOR = 5200;
const gameContainer = document.querySelector<HTMLDivElement>(".chart-bg")?.parentElement!;

function startAnimation() {
  gameContainer.classList.remove("crashed", "cashout");
  gameContainer.classList.add("running");
}

function stopAnimation(crashed: boolean) {
  gameContainer.classList.remove("running", "cashout");
  if (crashed) gameContainer.classList.add("crashed");
}

function cashoutAnimation(cashout: boolean) {
  gameContainer.classList.remove("running", "crashed");
  if (cashout) gameContainer.classList.add("cashout");
}
...
function setCrashPoint(value: number | null) {
  crashPointEl.textContent = value === null || !Number.isFinite(value) ? "--" : `${value.toFixed(2)}x`;
}
...
function startPolling() {
  ...
  pollTimer = window.setInterval(() => void pollStatus(), 100);
}
...
function animateMultiplier() {
  if (!activeGameId || roundStartTime === null) return;
  const elapsed = Date.now() - roundStartTime;
  let multiplier = Math.exp(elapsed / EXP_DIVISOR);

  if (fairnessProof && multiplier >= fairnessProof.crashPoint) {
    multiplier = fairnessProof.crashPoint;
    setMultiplier(multiplier);
    stopAnimation(true);
    activeGameId = null;
    setStatusText("Crashed");
    setMessage("error", `Busted at ${multiplier.toFixed(2)}x`);
    if (currentBet !== null) setEarnings(-currentBet);
    currentBet = null;
    if (!fairnessProof) void requestReveal();
    return;
  }
...
function setVerification(text: string, success: boolean | null) {
  ...
  verificationEl.classList.remove("success", "error");
  if (success === true) verificationEl.classList.add("success");
  if (success === false) verificationEl.classList.add("error");
}
...
async function verifyProof(proof: Proof) {
  try {
    ...
    setVerification(delta < 0.01 ? "Verified" : "Crash mismatch", delta < 0.01 ? true : false);
  } catch {
    setVerification("Verification unavailable", null);
  }
}
...
async function startRound() {
  if (activeGameId) return setMessage("error", "Round already running");
  const bet = parseBet();
  if (bet === null) return setMessage("error", "Enter a valid bet amount");
  if (balance !== null && bet > balance) return setMessage("error", "Not enough gems for that bet");
  ...
  try {
    ...
    if (!res.ok || data.error) {
      startBtn.disabled = false;
      return setMessage("error", String(data.error ?? "Unable to start round"));
    }
...
async function pollStatus() {
  if (!activeGameId || polling) return;
  polling = true;
  try {
    const res = await fetch(`${API}/status?gameId=${encodeURIComponent(activeGameId)}`);
    if (!res.ok) {
      if (res.status === 404) { stopPolling(); activeGameId = null; updateControlsForState("idle"); }
      return;
    }
    const data = await res.json();
    if (data.status === "running") { setStatusText("Running"); return; }
    ...
    if (data.status === "cashed") { updateControlsForState("cashed"); setStatusText("Cashed out"); cashoutAnimation(false); }
...
async function cashOut() {
  ...
  cashoutBtn.disabled = true;
  try {
    const res = await fetch(`${API}/cashout`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameId: activeGameId }) });
    const data = await res.json();
    if (!res.ok || data.error) { setMessage("error", String(data.error ?? "Cashout failed")); cashoutBtn.disabled = false; return; }
    ...
    if (typeof data.payout === "number" && currentBet !== null) setEarnings(data.payout - currentBet);
    if (typeof data.balance === "number") setBalance(data.balance);

    const proof: Proof | null = typeof data.serverSeed === "string" && typeof data.hash === "string" && typeof data.crashPoint === "number"
      ? { serverSeed: data.serverSeed, clientSeed: typeof data.clientSeed === "string" ? data.clientSeed : clientSeed, commit: typeof data.commit === "string" ? data.commit : commitEl.textContent ?? "", nonce: typeof data.nonce === "number" ? data.nonce : 0, hash: data.hash, crashPoint: data.crashPoint }
      : null;
    if (proof) { setCommitData(proof.commit, proof.clientSeed, proof.nonce); applyProof(proof); }

    currentBet = null;
  } catch (err) { setMessage("error", err instanceof Error ? err.message : "Cashout failed"); cashoutBtn.disabled = false; }
}

async function requestReveal() {
  if (!completedGameId) return setMessage("info", "No round to reveal");
  revealBtn.disabled = true;
  try {
    const res = await fetch(`${API}/reveal`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameId: completedGameId }) });
    const data = await res.json();
    if (!res.ok || data.error) { revealBtn.disabled = false; return setMessage("error", String(data.error ?? "Reveal failed")); }
  ...
  } catch (err) { setMessage("error", err instanceof Error ? err.message : "Reveal failed"); revealBtn.disabled = false; }
}

startBtn.addEventListener("click", () => void startRound());
cashoutBtn.addEventListener("click", () => void cashOut());
revealBtn.addEventListener("click", () => void requestReveal());

void ensureAuthenticated().then(profile => { if (!profile) return; void refreshBalance(); });
